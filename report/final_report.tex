...
\label{eq:lod}
\end{equation}
where $\sigma$ is the assumed current noise floor and $S$ is the low-concentration sensitivity~\cite{Shrivastava2011}. The detection criterion is
\begin{equation}
...
Experiment E converts the Debye-screening limitation into engineering requirements. Combining Eqs.~\ref{eq:gfet}--\ref{eq:debye} with the detection threshold gives the required coupling condition
\begin{equation}
...
Experiment A compares the M2 diffusion-only baseline with M2B directional-flow cases at $C=10$ pM. The reference flow case, $v_{in}=5\times10^{-7}$ m/s, gives approximately 8.73 pM sensor concentration at 6000 s. Diffusion-only transport gives approximately 4.98 pM at the same time. This is approximately a 1.75x increase in late-time sensor exposure. The time-to-50\% exposure decreases from about 6038 s to about 1839 s. This is the main positive transport result: directional lymph-node-inspired flow improves sensor exposure under the modeled assumptions.
...
The current-response range across placements is approximately 559.9--600.5 pA under flow and 348.2--533.3 pA under diffusion-only transport. Therefore, the late-time current spread is smaller under flow than under diffusion-only transport, even though placement still affects response timing.
...
Surface binding converts sensor concentration into bound molecule count. The local GFET-scale sensing area produces the count used for current response. The M4 current response increases with HER2 concentration and coupling efficiency. Under the current assumptions, the $\alpha=0.03$, 10 pA case reaches a simulated LOD of approximately 0.78 pM. This value is conditional and should not be generalized to all noise, coupling, or screening cases.
