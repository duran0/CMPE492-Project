...
Sensor placement affects response timing and current response. & Subcapsular/cortical placement is fastest and strongest under reference flow; late-time current range is 559.9 to 600.5 pA under flow and 348.2 to 533.3 pA under diffusion-only transport. & Figs.~\ref{fig:exp_b_exposure} and \ref{fig:exp_b_current} & Three placement regions are post-processed over the same transport model. & Placement sweep is not a full remeshed COMSOL sensor relocation study. & Medium\\
...
PBS-like Debye screening strongly reduces effective GFET signal for long receptor distances. & For PBS-like $\lambda_D=0.8$ nm and $d\geq5$ nm, 0 of 60 screened cases remain detectable. & Figs.~\ref{fig:exp_d_lod} and \ref{fig:exp_d_detectable} & Screened coupling model. & Debye lengths and distances are approximate feasibility assumptions. & Medium\\
